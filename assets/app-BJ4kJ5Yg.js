...
import{_ as le}from"./index-CzAWk2Jy.js";async function de(c,q={}){const{windowManager:B,vfs:x,themeManager:S,appLoader:F,loader:E,panelManager:O}=c,{IconHelper:b}=await le(async()=>{const{IconHelper:e}=await import("./index-CzAWk2Jy.js").then(a=>a.i);return{IconHelper:e}},__vite__mapDeps([0,1]));let g={};const _=async()=>{try{const e=await x.readFile("~/.config/appearance.json");e&&(g=JSON.parse(e))}catch{}},A=async()=>{await x.writeFile("~/.config/appearance.json",JSON.stringify(g,null,2)),document.dispatchEvent(new CustomEvent("reload-appearance"))};await _();let T="appearance";q.args&&q.args.length>0&&(T=q.args[0]),q.section&&(T=q.section);const P=B.windows.get("system-settings");if(P){B.focusWindow("system-settings"),setTimeout(()=>{const e=P.frame.querySelector(`.settings-nav-item[data-section="${T}"]`);e&&(e.click(),e.dispatchEvent(new Event("click",{bubbles:!0})))},50);return}const k=document.createElement("div");k.style.height="100%",k.style.display="flex",k.style.background="var(--bg-surface)",k.style.color="var(--text-primary)",k.style.fontFamily="var(--font-main)",k.innerHTML=`
  ...
  `;const M=B.createWindow({id:"system-settings",title:"System Settings",icon:"settings,⚙️",width:800,height:600,content:k,onClose:()=>{C==="menu"&&window.dispatchEvent(new CustomEvent("everest:close-menu"))}}),$=k.querySelector("#settings-sidebar"),N=k.querySelector("#settings-header"),i=k.querySelector("#settings-body"),W=k.querySelectorAll(".settings-nav-item"),G=()=>{$.querySelector('[data-section="appearance"]').innerHTML=`${b.getIcon("appearance,🎨",{size:16})} Appearance`,$.querySelector('[data-section="panel"]').innerHTML=`${b.getIcon("panel-color,🚥",{size:16})} Panel`,$.querySelector('[data-section="menu"]').innerHTML=`${b.getIcon("menu-color,🌿",{size:16})} Menu`,$.querySelector('[data-section="desktop"]').innerHTML=`${b.getIcon("desktop,🖥️",{size:16})} Desktop`,$.querySelector('[data-section="display"]').innerHTML=`${b.getIcon("monitor-color,🖥️",{size:16})} Display`,$.querySelector('[data-section="extensions"]').innerHTML=`${b.getIcon("plugin-color,🧩",{size:16})} Extensions`,$.querySelector('[data-section="startup"]').innerHTML=`${b.getIcon("startup,🚀",{size:16})} Startup`,$.querySelector('[data-section="storage"]').innerHTML=`${b.getIcon("storage,💽",{size:16})} Storage`,$.querySelector('[data-section="users"]').innerHTML=`${b.getIcon("user-color,👥",{size:16})} Users`,$.querySelector('[data-section="about"]').innerHTML=`${b.getIcon("system-color,ℹ️",{size:16})} About`};window.addEventListener("icon-theme-changed",()=>{G(),j(C)}),window.addEventListener("theme-changed",()=>{j(C)});let C=T,z=null;const D=async e=>{if(e==="menu"){window.dispatchEvent(new CustomEvent("everest:open-menu")),z||(z={left:M.frame.style.left,top:M.frame.style.top});try{const a=await x.readFile("~/.config/menu.json"),o=(a?JSON.parse(a):{menuWidth:420}).menuWidth||420;M.frame.style.left=o+60+"px";const d=window.innerWidth-M.frame.offsetWidth-20;parseFloat(M.frame.style.left)>d&&(M.frame.style.left=d+"px")}catch{M.frame.style.left="450px"}}else C==="menu"&&(window.dispatchEvent(new CustomEvent("everest:close-menu")),z&&(M.frame.style.left=z.left,M.frame.style.top=z.top,z=null));C=e,W.forEach(a=>{a.classList.toggle("active",a.dataset.section===e)}),N.textContent=e.charAt(0).toUpperCase()+e.slice(1),j(e)},J=()=>{const e=document.documentElement,a={height:g.panelHeight!==void 0?g.panelHeight:48,blur:g.panelBlur!==void 0?g.panelBlur:20,opacity:g.panelOpacity!==void 0?g.panelOpacity:.92,marginY:g.panelMarginY!==void 0?g.panelMarginY:12,marginX:g.panelMarginX!==void 0?g.panelMarginX:60,radius:g.panelRadius!==void 0?g.panelRadius:12,color:g.panelColor||"#1e1e1e",borderColor:g.panelBorderColor||"#6496ff",borderOpacity:g.panelBorderOpacity!==void 0?g.panelBorderOpacity:.3};i.innerHTML=`
    ...
    `;const m=(t,n)=>{if(!t||!t.startsWith("#"))return t;const y=parseInt(t.slice(1,3),16)||0,w=parseInt(t.slice(3,5),16)||0,L=parseInt(t.slice(5,7),16)||0;return`rgba(${y}, ${w}, ${L}, ${n})`},o=async(t,n)=>{g[t]=n,await A(),t==="panelRadius"?e.style.setProperty("--panel-radius",n+"px"):t==="panelBlur"?e.style.setProperty("--panel-blur",n+"px"):t==="panelOpacity"||t==="panelColor"?e.style.setProperty("--bg-panel-rgba",m(g.panelColor||"#1e1e1e",g.panelOpacity!==void 0?g.panelOpacity:.92)):t==="panelHeight"?(e.style.setProperty("--panel-height",n+"px"),c.panelManager&&(c.panelManager.height=parseInt(n))):t==="panelMarginY"?e.style.setProperty("--panel-margin-y",n+"px"):t==="panelMarginX"?e.style.setProperty("--panel-margin-x",n+"px"):(t==="panelBorderColor"||t==="panelBorderOpacity")&&e.style.setProperty("--panel-border-rgba",m(g.panelBorderColor||"#6496ff",g.panelBorderOpacity!==void 0?g.panelBorderOpacity:.3))},d=i.querySelector("#panel-position");d&&d.addEventListener("change",t=>{c.panelManager&&(c.panelManager.position=t.target.value)});const v=i.querySelector("#panel-autohide");v&&v.addEventListener("change",t=>{c.panelManager&&(c.panelManager.autoHide=t.target.checked)});const l=i.querySelector("#panel-show-app-name");l&&l.addEventListener("change",t=>{c.panelManager&&(c.panelManager.showAppName=t.target.checked)});const s=i.querySelector("#panel-icon-size");s&&s.addEventListener("input",t=>{const n=parseInt(t.target.value);i.querySelector("#val-panel-icon-size").textContent=n+"px",c.panelManager&&(c.panelManager.iconSize=n)}),i.querySelector("#panel-color").addEventListener("input",t=>o("panelColor",t.target.value)),i.querySelector("#opacity-range").addEventListener("input",t=>{const n=parseInt(t.target.value);i.querySelector("#opacity-val").textContent=n+"%",o("panelOpacity",n/100)}),i.querySelector("#blur-range").addEventListener("input",t=>{const n=parseInt(t.target.value);i.querySelector("#blur-val").textContent=n+"px",o("panelBlur",n)}),i.querySelector("#border-color").addEventListener("input",t=>o("panelBorderColor",t.target.value)),i.querySelector("#border-opacity").addEventListener("input",t=>{const n=parseInt(t.target.value);i.querySelector("#border-opacity-val").textContent=n+"%",o("panelBorderOpacity",n/100)}),i.querySelector("#height-range").addEventListener("input",t=>{const n=parseInt(t.target.value);i.querySelector("#height-val").textContent=n+"px",o("panelHeight",n)});const u=i.querySelector("#margin-y-range"),f=i.querySelector("#margin-x-range"),p=()=>{const t=document.getElementById("everest-panel");t&&t.classList.toggle("is-dock",parseInt(u.value)>0||parseInt(f.value)>0)};u.addEventListener("input",t=>{const n=parseInt(t.target.value);i.querySelector("#margin-y-val").textContent=n+"px",o("panelMarginY",n),p()}),f.addEventListener("input",t=>{const n=parseInt(t.target.value);i.querySelector("#margin-x-val").textContent=n+"px",o("panelMarginX",n),p()}),i.querySelector("#radius-range").addEventListener("input",t=>{const n=parseInt(t.target.value);i.querySelector("#radius-val").textContent=n+"px",o("panelRadius",n)});const h=i.querySelector("#panel-mode-group"),r=()=>{const t=parseInt(u.value)>0||parseInt(f.value)>0;h.querySelectorAll("button").forEach(n=>{const y=n.dataset.mode==="dock"&&t||n.dataset.mode==="full"&&!t;n.className=y?"btn-primary btn-sm":"btn-secondary btn-sm"})};r(),h.querySelectorAll("button").forEach(t=>{t.onclick=()=>{const n=i.querySelector("#height-range"),y=i.querySelector("#radius-range");t.dataset.mode==="full"?(u.value=0,f.value=0,n.value=42,y.value=0):(u.value=12,f.value=60,n.value=48,y.value=12),u.dispatchEvent(new Event("input")),f.dispatchEvent(new Event("input")),n.dispatchEvent(new Event("input")),y.dispatchEvent(new Event("input")),r()}})},j=async e=>{i.innerHTML="",e==="appearance"?H():e==="panel"?J():e==="menu"?await Z():e==="desktop"?await Q():e==="display"?ee():e==="extensions"?te():e==="startup"?await U():e==="storage"?await K():e==="users"?ie():e==="about"&&ne()},U=async()=>{i.innerHTML=`
    ...
    `;const e=i.querySelector("#startup-list");let a=[];try{const d=await x.readFile("~/.config/startup.json");d&&(a=JSON.parse(d))}catch{}const m=F.getApps();(()=>{e.innerHTML="",m.forEach(d=>{const v=a.includes(d.id),l=document.createElement("div");l.className="settings-card",l.style.margin="0",l.innerHTML=`
    ...
    `;const o=i.querySelector("#storage-val"),d=i.querySelector("#storage-bar"),v=i.querySelector("#storage-file-count"),l=i.querySelector("#backup-status"),s=p=>{l.style.display="block",l.textContent=p};(async()=>{let p=0,h=0;const r=async y=>{try{const w=await x.readdir(y);for(const L of w)L.type==="dir"?await r(L.path):(p+=L.size||0,h++)}catch{}};await r("/");const t=a?R.storageLimitServer:R.storageLimitLocal,n=Math.min(100,p/t*100);o.textContent=`${(p/1024/1024).toFixed(2)} MB of ${(t/1024/1024).toFixed(0)} MB used`,d.style.width=n+"%",v.textContent=`${h} files indexed`})(),i.querySelector("#btn-export-vfs").onclick=async()=>{try{s("⏳ Collecting files...");const p=await V(),h=JSON.stringify(p,null,2),r=new Blob([h],{type:"application/json"}),t=URL.createObjectURL(r),n=document.createElement("a");n.href=t,n.download=`everest-backup-${new Date().toISOString().split("T")[0]}.json`,n.click(),URL.revokeObjectURL(t),s(`✅ Backup downloaded — ${p.fileCount} files, ${(r.size/1024).toFixed(1)} KB`)}catch(p){s(`❌ Export failed: ${p.message}`)}};const f=i.querySelector("#import-vfs-file");i.querySelector("#btn-import-vfs").onclick=()=>f.click(),f.onchange=async p=>{const h=p.target.files[0];if(!h)return;const r=new FileReader;r.onload=async t=>{try{const n=JSON.parse(t.target.result);let y;if(Array.isArray(n))y={files:n,timestamp:"legacy",os:"unknown"};else if(n.files)y=n;else throw new Error("Invalid backup format");if(confirm(`Restore ${y.files.length} items from ${y.os||"unknown"} (${y.timestamp||"unknown"})? This will write files to the active storage backend.`)){s("⏳ Restoring files...");const{restored:w,errors:L}=await Y(y);s(`✅ Restored ${w} items${L>0?`, ${L} errors`:""}. Reloading...`),setTimeout(()=>location.reload(),1500)}}catch(n){s(`❌ Import failed: ${n.message}`)}},r.readAsText(h)},i.querySelector("#btn-reset-system").onclick=async()=>{if(confirm("Are you absolutely sure you want to reset the system? ALL DATA WILL BE LOST.")){const p=indexedDB.deleteDatabase("PlaygroundVFS");p.onsuccess=()=>location.reload(),p.onerror=()=>alert("Failed to clear database.")}}},Z=async()=>{let e={icon:"menu",showLabel:!0,menuWidth:420,menuOpacity:.85,menuBlur:20,showCategoryIcons:!0,enableSearch:!0,iconSize:28};try{const s=await x.readFile("~/.config/menu.json");s&&(e={...e,...JSON.parse(s)})}catch{}const a=async()=>{await x.writeFile("~/.config/menu.json",JSON.stringify(e,null,2)),document.dispatchEvent(new CustomEvent("reload-menu-settings"))};i.innerHTML=`
    ...
    `;const m=i.querySelector("#icon-preview"),o=i.querySelector("#icon-selector"),d=i.querySelector("#custom-icon-grid"),v=()=>{const s=e.icon;m.innerHTML=b.getIcon(s,{size:28})};v(),m.onclick=()=>{const s=o.style.display==="block";o.style.display=s?"none":"block",m.style.borderColor=s?"var(--border)":"var(--accent)",s||l()};const l=async()=>{d.innerHTML='<div style="grid-column: span 6; font-size:10px; color:var(--text-tertiary);">Scanning...</div>';try{const s=await x.readdir("~/images/icons");d.innerHTML="",s.forEach(u=>{if(u.type==="dir"||!u.name.match(/\.(png|jpg|jpeg|gif|svg|webp)$/i))return;const f=document.createElement("div");f.style.cssText=`
            height: 40px;
            border-radius: 6px;
            background: var(--bg-surface-hover);
            border: 1px solid var(--border);
            cursor: pointer;
            background-image: url("${x.getFsPath(u.path)}");
            background-size: contain;
            background-position: center;
            background-repeat: no-repeat;
          `,f.onclick=()=>{e.icon=u.path,v(),a()},d.appendChild(f)}),s.length===0&&(d.innerHTML='<div style="grid-column: span 6; font-size:10px; color:var(--text-tertiary);">No icons in ~/images/icons</div>')}catch{d.innerHTML='<div style="grid-column: span 6; font-size:10px; color:var(--text-tertiary);">Failed to scan icons</div>'}};i.querySelectorAll(".emoji-item").forEach(s=>{s.onclick=()=>{e.icon=s.textContent,v(),a()}}),i.querySelector("#btn-browse-custom").onclick=async()=>{const s=await c.filePicker.pickFile({title:"Select Menu Icon",initialPath:"~/images/icons",filter:u=>u.type==="dir"||u.name.match(/\.(png|jpg|jpeg|gif|svg|webp)$/i)});s&&(e.icon=s,v(),a())},i.querySelector("#btn-theme-default").onclick=()=>{e.icon="menu",v(),a()},i.querySelector("#show-label").onchange=s=>{e.showLabel=s.target.checked,a()},i.querySelector("#icon-size-range").oninput=s=>{const u=s.target.value;e.iconSize=parseInt(u),i.querySelector("#icon-size-val").textContent=u+"px",a()},i.querySelector("#width-range").oninput=s=>{const u=s.target.value;e.menuWidth=parseInt(u),i.querySelector("#width-val").textContent=u+"px",a()},i.querySelector("#menu-blur-range").oninput=s=>{const u=s.target.value;e.menuBlur=parseInt(u),i.querySelector("#menu-blur-val").textContent=u+"px",a()},i.querySelector("#menu-opacity-range").oninput=s=>{const u=s.target.value;e.menuOpacity=parseInt(u)/100,i.querySelector("#menu-opacity-val").textContent=u+"%",a()},i.querySelector("#enable-search").onchange=s=>{e.enableSearch=s.target.checked,a()},i.querySelector("#show-categories").onchange=s=>{e.showCategoryIcons=s.target.checked,a()}},H=()=>{const e={shellTheme:g.shellTheme||"dark",appTheme:g.appTheme||"dark",ctxColor:g.contextMenuColor||"#19191e",ctxOpacity:g.contextMenuOpacity!==void 0?g.contextMenuOpacity:.97,ctxBlur:g.contextMenuBlur!==void 0?g.contextMenuBlur:24,radius:g.panelRadius||10},a=S.currentTheme.replace("-dark",""),m=S.themes.has(a)&&S.themes.has(`${a}-dark`);i.innerHTML=`
    ...
    `;const o=document.documentElement,d=(r,t)=>{if(!r.startsWith("#"))return r;const n=parseInt(r.slice(1,3),16)||0,y=parseInt(r.slice(3,5),16)||0,w=parseInt(r.slice(5,7),16)||0;return`rgba(${n}, ${y}, ${w}, ${t})`};i.querySelector("#master-mode-group").querySelectorAll("button").forEach(r=>{r.onclick=()=>{const t=r.dataset.mode;S.setMode(t),H()}});const l=async(r,t)=>{g[r]=t,await A(),r==="contextMenuColor"||r==="contextMenuOpacity"?g.appTheme!=="light"&&o.style.setProperty("--bg-context-menu-rgba",d(g.contextMenuColor||"#19191e",g.contextMenuOpacity!==void 0?g.contextMenuOpacity:.97)):r==="contextMenuBlur"?o.style.setProperty("--context-menu-blur",t+"px"):r==="panelRadius"&&(o.style.setProperty("--radius-main",t+"px"),o.style.setProperty("--radius-md",t+"px"))};i.querySelector("#ctx-color").addEventListener("input",r=>l("contextMenuColor",r.target.value)),i.querySelector("#ctx-opacity").addEventListener("input",r=>{const t=parseInt(r.target.value);i.querySelector("#ctx-opacity-val").textContent=t+"%",l("contextMenuOpacity",t/100)}),i.querySelector("#ctx-blur").addEventListener("input",r=>{const t=parseInt(r.target.value);i.querySelector("#ctx-blur-val").textContent=t+"px",l("contextMenuBlur",t)}),i.querySelector("#radius-range").addEventListener("input",r=>{const t=parseInt(r.target.value);i.querySelector("#radius-val").textContent=t+"px",l("panelRadius",t)});const u=i.querySelector("#theme-grid"),f=()=>{u.innerHTML="";const r=S.getThemes();r.filter(n=>{if(n.id.endsWith("-dark")){const y=n.id.replace("-dark","");return!r.some(w=>w.id===y)}return!0}).forEach(n=>{const y=document.createElement("div"),w=S.currentTheme===n.id||S.currentTheme===`${n.id}-dark`,ae=S.preferredMode==="dark"&&S.themes.has(`${n.id}-dark`)?`${n.id}-dark`:n.id,I=S.themes.get(ae)||n,se=I.variables["--bg-surface-rgb"]?`rgb(${I.variables["--bg-surface-rgb"]})`:I.mode==="light"?"#f0f0f0":"#1e1e1e",re=I.variables["--text-primary-rgb"]?`rgb(${I.variables["--text-primary-rgb"]})`:I.mode==="light"?"#222":"#eee",oe=I.variables["--accent-rgb"]?`rgb(${I.variables["--accent-rgb"]})`:"#3584e4";y.style.cssText=`
          background: ${se};
          color: ${re};
        ...
        `,y.innerHTML=`
          ...
          <span style="font-size:11px; font-weight:700; text-align:center; letter-spacing:0.3px;">${n.name.replace("-Dark","").replace("-Y","Y")}</span>
        ...
        `,y.onmouseover=()=>{w||(y.style.transform="translateY(-4px)")},y.onmouseout=()=>{w||(y.style.transform="scale(1)")},y.addEventListener("click",()=>{S.applyTheme(n.id,{ignoreMode:!1}),H()}),u.appendChild(y)})},p=i.querySelector("#icon-theme-grid"),h=()=>{p.innerHTML="",S.getIconThemes().forEach(r=>{const t=document.createElement("div"),n=S.currentIconTheme===r.id;t.style.cssText=`
          background: var(--bg-card);
          border: 2px solid ${n?"var(--accent)":"var(--border)"};
          border-radius: 14px;
          padding: 16px;
          cursor: pointer;
          display: flex;
          ...
          gap: 12px;
          transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1);
          box-shadow: ${n?"0 8px 24px rgba(0,0,0,0.15)":"none"};
        ...
        `;const y=b.getIcon("folder,📁",{size:32});t.innerHTML=`
          ...
          ${n?'<div style="position:absolute; top:8px; right:8px; width:8px; height:8px; background:var(--accent); border-radius:50%; box-shadow: 0 0 8px var(--accent);"></div>':""}
        `,t.onmouseover=()=>{n||(t.style.transform="translateY(-4px)")},t.onmouseout=()=>{n||(t.style.transform="scale(1)")},t.addEventListener("click",()=>{S.applyIconTheme(r.id),h(),window.dispatchEvent(new CustomEvent("icon-theme-changed",{detail:{theme:r}}))}),p.appendChild(t)})};f(),h()},Q=async()=>{i.innerHTML=`
    ...
    `;const e=i.querySelector("#desktop-content"),a=i.querySelector("#tab-bg"),m=i.querySelector("#tab-icons"),o=async()=>{e.innerHTML=`
      ...
      `;const v=e.querySelector("#grad-grid");["linear-gradient(135deg, #2c3e50, #3498db)","linear-gradient(135deg, #1e130c, #9a8478)","linear-gradient(135deg, #43cea2, #185a9d)","linear-gradient(135deg, #ff4b1f, #ff9068)","#242424","#1a1a1a","#3584e4","#26a269"].forEach(f=>{const p=document.createElement("div");p.style.height="50px",p.style.borderRadius="6px",p.style.background=f,p.style.cursor="pointer",p.style.border="2px solid transparent",p.onclick=()=>{c.desktopSettings.settings.background=f,c.desktopSettings._applyBackground(),c.desktopSettings._save(),v.querySelectorAll("div").forEach(h=>h.style.borderColor="transparent"),p.style.borderColor="var(--accent)"},v.appendChild(p)});const s=e.querySelector("#wallpaper-grid"),u=async()=>{s.innerHTML='<div style="grid-column: 1/-1; text-align:center; padding:20px; color:var(--text-tertiary);">Scanning for images...</div>';const f=["~/images/backgrounds"];let p=[];for(const h of f)try{(await x.readdir(h)).forEach(t=>{if(t.type!=="dir"&&t.name.match(/\.(png|jpg|jpeg|webp)$/i)){let n=t.path||`${h}/${t.name}`;const y=x.getFsPath(n);p.push({name:t.name,path:y})}})}catch(r){console.warn(`Failed to scan wallpapers in ${h}`,r)}if(s.innerHTML="",p.length===0){s.innerHTML='<div style="grid-column: 1/-1; text-align:center; padding:20px; color:var(--text-tertiary);">No backgrounds found.</div>';return}p.forEach(h=>{const r=document.createElement("div");r.className="wallpaper-item",r.style.cssText=`
            height: 100px;
            border-radius: 6px;
    ...
    `;const m=i.querySelector("#scaling-select");m.onchange=l=>{e.scaling=l.target.value;const s=parseInt(l.target.value)/100;document.documentElement.style.setProperty("--system-scale",s),a()};const o=i.querySelector("#night-light-toggle");o.onchange=l=>{e.nightLight=l.target.checked,document.body.classList.toggle("night-light",e.nightLight),a()};const d=i.querySelector("#panel-pos-group"),v=()=>{d.querySelectorAll("button").forEach(l=>{const s=l.dataset.pos===O.position;l.className=s?"btn-primary btn-sm":"btn-secondary btn-sm"})};v(),d.querySelectorAll("button").forEach(l=>{l.onclick=()=>{O.position=l.dataset.pos,v()}})},te=()=>{i.innerHTML=`
    ...
    `,i.querySelector("#btn-dev-docs").onclick=()=>{document.dispatchEvent(new CustomEvent("launch-app",{detail:{id:"web-browser",args:["/home/user/Documents/developer-guides.html"]}}))}};$.addEventListener("click",e=>{const a=e.target.closest(".settings-nav-item");a&&D(a.dataset.section)}),D(T)}export{de as launch};
