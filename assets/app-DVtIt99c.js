import{I as a}from"./index-BX5Da8PK.js";function z(r,d={}){const{windowManager:p,vfs:g}=r,i=d.path||"",e=document.createElement("div");e.style.padding="0",e.style.height="100%",e.style.display="flex",e.style.flexDirection="column",e.style.background="var(--bg-surface)",e.style.color="var(--text-primary)",e.style.fontFamily="Inter, system-ui, sans-serif";const m=i?i.substring(i.lastIndexOf("/")+1):"No image loaded";e.innerHTML=`
    <!-- Toolbar -->
  ...
  `,p.createWindow({id:`image-viewer-${Date.now()}`,title:"Image Viewer",icon:"image",width:600,height:460,content:e});const l=e.querySelector("#iv-img"),y=e.querySelector("#iv-zoom-in"),f=e.querySelector("#iv-zoom-out"),v=e.querySelector("#iv-rotate");let t=1,o=0;const s=()=>{l.style.transform=`scale(${t}) rotate(${o}deg)`};y.addEventListener("click",()=>{t=Math.min(4,t+.2),s()}),f.addEventListener("click",()=>{t=Math.max(.2,t-.2),s()}),v.addEventListener("click",()=>{o=(o+90)%360,s()});const{filePicker:x}=r,u=e.querySelector("#iv-open"),h=e.querySelector("#iv-title"),c=n=>{let b=g.resolvePath(n).replace(/^~/,"/home/user");l.src=`/fs${b}`,h.textContent=n.split("/").pop(),t=1,o=0,s()};u.addEventListener("click",async()=>{const n=await x.pickFile({title:"Open Image",filter:[".png",".jpg",".jpeg",".gif",".svg",".webp"]});n&&c(n)}),i&&c(i)}export{z as launch};
