import{I as i}from"./index-BX5Da8PK.js";function F(h,w={}){const{windowManager:u,vfs:q,filePicker:T}=h,v=w.path||"",t=document.createElement("div");t.style.cssText=`
    height: 100%;
    display: flex;
  ...
  `;const k=u.createWindow({id:`video-player-${Date.now()}`,title:"Video Player",icon:"video",width:800,height:500,content:t}),e=t.querySelector("#vp-video"),n=t.querySelector("#vp-play"),s=t.querySelector("#vp-progress"),y=t.querySelector("#vp-volume"),z=t.querySelector("#vp-current-time"),f=t.querySelector("#vp-duration"),S=t.querySelector("#vp-fullscreen"),a=t.querySelector("#vp-top"),p=t.querySelector("#vp-bottom"),m=t.querySelector("#vp-container"),$=t.querySelector("#vp-title");let r=!0;const g=o=>{if(!o)return;let d=q.resolvePath(o).replace(/^~/,"/home/user");e.src=`/fs${d}`;const l=o.split("/").pop();$.textContent=l,u.setTitle(k.id,`Video - ${l}`)},c=o=>{if(isNaN(o))return"0:00";const d=Math.floor(o/60),l=Math.floor(o%60);return`${d}:${l<10?"0":""}${l}`};let x;const b=()=>{a.style.opacity="1",p.style.opacity="1",clearTimeout(x),x=setTimeout(()=>{r&&(a.style.opacity="0",p.style.opacity="0")},2500)};t.onmousemove=b,t.onmouseleave=()=>{a.style.opacity="0",p.style.opacity="0"},n.onclick=()=>{r?(e.pause(),n.innerHTML=i.getIcon("play",{size:18}),r=!1):(e.play().catch(o=>console.error(o)),n.innerHTML=i.getIcon("pause",{size:18}),r=!0)},e.onclick=()=>n.onclick(),e.addEventListener("timeupdate",()=>{e.duration&&(s.value=e.currentTime/e.duration*100,z.textContent=c(e.currentTime),f.textContent=c(e.duration))}),e.addEventListener("loadedmetadata",()=>{f.textContent=c(e.duration),b()}),s.oninput=()=>{e.duration&&(e.currentTime=s.value/100*e.duration)},y.oninput=()=>{e.volume=y.value/100},S.onclick=()=>{e.requestFullscreen?e.requestFullscreen():m.requestFullscreen&&m.requestFullscreen()},t.querySelector("#vp-open").onclick=async()=>{const o=await T.pickFile({title:"Open Video File",filter:[".mp4",".webm",".mov",".mkv"]});o&&(g(o),r||n.onclick())},v&&g(v)}export{F as launch};
