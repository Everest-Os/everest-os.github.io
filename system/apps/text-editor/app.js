...
export async function launch(ctx, options = {}) {
  ...
  const loadFile = async (path) => {
    try {
      ...
      // Clear previous hex buttons
      const oldBtn = editorContainer.querySelector('.hex-fallback-btn');
      if (oldBtn) oldBtn.remove();

      if (text instanceof Blob) {
        textarea.value = "Unsupported Binary File\n\nThis file format is not natively supported as plain text.\nWould you like to open it in the Hex Editor?";
        textarea.readOnly = true;
        textarea.style.opacity = '0.5';
        content.querySelector('#btn-save').disabled = true;
        
        const hexBtn = document.createElement('button');
        hexBtn.className = 'btn-primary hex-fallback-btn';
        hexBtn.style.position = 'absolute';
        hexBtn.style.top = '60%';
        hexBtn.style.left = '50%';
        hexBtn.style.transform = 'translate(-50%, -50%)';
        hexBtn.style.padding = '12px 24px';
        hexBtn.style.display = 'flex';
        hexBtn.style.alignItems = 'center';
        hexBtn.style.gap = '8px';
        hexBtn.innerHTML = `${IconHelper.getIcon('application-octet-stream', { size: 16 })} Open in Hex Editor`;
        hexBtn.onclick = () => {
          document.dispatchEvent(new CustomEvent('launch-app', { detail: { id: 'hex-editor', args: [path] } }));
        };
        editorContainer.appendChild(hexBtn);
      } else {
        textarea.value = text;
